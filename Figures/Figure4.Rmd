...
```{r}
scheme <- magick::image_read_svg("main_sim_scheme.svg", width=2880, height=1620)
scheme <- magick::image_ggplot(scheme) + labs(tag="A") +
...
lfcDist <- ggplot(lfc, aes(x=enr, y=lfc))+
  ...
  scale_color_viridis(option="A", name="point density ", breaks=c(1000,4000,7000)) +
...
plot_grid((scheme + free(lfcDist) + plot_layout(widths=c(4,3.3))), es_perform, nrow=2, rel_heights=c(1.2,3))
dev.off()
```
