...
**A:** Number of peaks, proportion of ChIP-seq and/or ATAC-seq peaks with a motif, and fold-enrichment of the ChIP-seq peaks for the motif across the four TFs. **B-C:** Log2 fold-enrichment of all tested motifs among the ChIP-seq peaks (relative to ATAC-seq peaks), with the true motif labeled in black. The dots are colored either by co-occurence with the true motif (**B**) or by GC content (**C**). **D:** GC content distributions of the ChIP-seq peaks of each TF.
...
**`r getFigNb()` : Distributions of fragment counts and variance of the simulations**
**A:** Per peak fragment count distributions of the original (denoted ”0”) and simulated samples with 1-fold and 3-fold perturbation strengths, for an example TF (CEBPB) across both paradigms. **B:** Mean-variance relationship of the original (denoted "0") and simulated samples.
...
**`r getFigNb()` : Heatmap of performance of variants of top methods on the semi-simulated datasets.**
...
scale_fill_manual(values=c("#fa8d24f9", "#952758ff"))+
xlab("GC content")+
...
tlr + tls + plot_layout(guides="collect")
```

**`r getFigNb()` : Estimated precisions per dataset**
**A:** Colored point map showing precision estimates obtained per dataset and method on the 11 real datasets, point sizes indicate if the true TF was found to be significant by the respective method. **B:** Heatmap showing the average precision and pertubation strength (across TFs and paradigms, no GC or FL biases included) and method.