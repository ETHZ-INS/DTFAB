...
6 baseline ATAC-seq lymphoblastoid cell lines (LCLs) are divided into two groups (**B**).
...
the fragments in one of the groups (**A**). The extent of downsampling is based on the 
...
```{r s2, fig.width=16, fig.height=12}
...
lfcDist+motifPlot+coRankSim+coRankGC+peakGCPlot+plot_layout(design=layout) & 
...
**`r getFigNb()` : Overview of the simulated TFs and effect paradigms. **
**A:** Relationship between log2FC and peak enrichment under the two simulation paradigms.
**B:** Number of peaks, proportion of ChIP and/or ATAC peaks with a motif, and fold-enrichment of the ChIP-peaks for the motif across the four TFs.
**C-D:** Enrichment of all tested motifs among the ChIP peaks (relative to ATAC peaks), with the true motif labeled in black. The dots are colored either by co-occurence with the true motif (**C**) or by GC content (**D**).
**E:** GC content distributions of the ChIP-peaks of each TF.
